...
Profile pictures are not uploaded. The static build fetches six real Fallout 4 perk images from the pinned public source documented in `PROFILE_ICON_ATTRIBUTION.md`, verifies their Git blob SHAs, and deploys them locally. Four committed Fallout 4 faction icons and five committed Commonwealth Online images in `assets/profile-images/` are deployed alongside them. The browser and database only accept those local paths. The database constraint is the enforcement boundary, so direct API requests cannot store arbitrary avatar URLs.
