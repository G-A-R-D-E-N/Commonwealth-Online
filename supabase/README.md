...
Profile avatars are intentionally not backed by Supabase Storage. `public.profiles.avatar_url` is restricted by a database check constraint to the built-in Fallout 4 themed icon paths, and `handle_new_user()` ignores provider avatar URLs outside that allow list.
